function[A_coup,B_coup,C_coup,A_aux,B_aux,C_aux,Q_red_y,Q_red_u,Q_exp1_y,Q_exp2_y,Q_exp_u] = Truncated_Normal_Distribution(A,B,C,tau_real_A,tau_im_A,n_aux,m_aux,p_aux,theta_u,theta_y)
    %% Auxiliary matrix B:
    ...
    %%
    Q_red_y  = GenerateFullRankMatrix(mu_C, sqrt(gamma_C), theta_y, p_sys);
    
    Q_red_u  = GenerateFullRankMatrix(mu_B, sqrt(gamma_B), theta_u, m_sys);
    
    Q_exp1_y = GenerateFullRankMatrix(mu_C, sqrt(gamma_C), n_aux, theta_y);
    
    Q_exp2_y = GenerateFullRankMatrix(mu_C, sqrt(gamma_C), p_aux, theta_y);
    
    Q_exp_u  = GenerateFullRankMatrix(mu_B, sqrt(gamma_B), n_aux, theta_u);
...
function Q = GenerateFullRankMatrix(mu,sigma,nRows,nCols)

r = min(nRows,nCols);

while true

    Q = mu + sigma*randn(nRows,nCols);
